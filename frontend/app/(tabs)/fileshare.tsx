...
// ScheduledTask, PRESETS, ScheduleSection removed — /api/scheduler not on this server
// ─── MAIN SCREEN ──────────────────────────────────────────────
interface _ScheduledTask_removed {
  id: string;
  name: string;
  script: string;
  language: string;
  ...
  nextRun: number;
  runCount: number;
  lastStatus: 'ok' | 'error' | 'timeout' | 'blocked' | 'pending';
  lastOutput: string;
}

const _PRESETS_removed: Array<{ name: string; script: string; interval: number; unit: 'minutes'|'hours'|'days'; icon: string }> = [
  {
    name: 'Clean Temp Files',
    script: `import os, shutil, tempfile\ntmp = tempfile.gettempdir()\ncount = 0\nfor f in os.listdir(tmp):\n    try:\n        p = os.path.join(tmp, f)\n        if os.path.isfile(p): os.remove(p); count += 1\n        elif os.path.isdir(p): shutil.rmtree(p, ignore_errors=True); count += 1\n    except: pass\nprint(f"Cleaned {count} temp items")`,
    interval: 6, unit: 'hours', icon: '🧹',
  },
  {
    name: 'PC Health Check',
    script: `import psutil\ncpu = psutil.cpu_percent(interval=1)\nram = psutil.virtual_memory().percent\ndisk = psutil.disk_usage('/').percent if not __import__('platform').system()=='Windows' else psutil.disk_usage('C:\\\\').percent\nprint(f"CPU: {cpu}% | RAM: {ram}% | Disk: {disk}%")\nif cpu > 90: print("WARNING CPU HIGH")\nif ram > 90: print("WARNING RAM HIGH")\nif disk > 90: print("WARNING DISK FULL")`,
    interval: 1, unit: 'hours', icon: '💓',
  },
  {
    name: 'Empty Recycle Bin',
    script: `import subprocess, platform\nif platform.system() == 'Windows':\n    subprocess.run(['powershell', '-Command', 'Clear-RecycleBin -Force -ErrorAction SilentlyContinue'], capture_output=True)\n    print("Recycle bin emptied")\nelse:\n    import shutil, os\n    trash = os.path.expanduser('~/.local/share/Trash/files')\n    if os.path.exists(trash): shutil.rmtree(trash, ignore_errors=True); os.makedirs(trash, exist_ok=True)\n    print("Trash emptied")`,
    interval: 1, unit: 'days', icon: '🗑️',
  },
  {
    name: 'Disk Space Monitor',
    script: `import psutil\ndisk = psutil.disk_usage('C:\\\\' if __import__('platform').system()=='Windows' else '/')\nfree_gb = disk.free / (1024**3)\npct = disk.percent\nprint(f"Disk: {pct}% used | {free_gb:.1f} GB free")\nif pct > 90: print("CRITICAL: Less than 10% disk space remaining!")\nelif pct > 80: print("WARNING: Disk space getting low")\nelse: print("Disk space OK")`,
    interval: 4, unit: 'hours', icon: '💾',
  },
];

function _ScheduleSection_removed({ isConnected, serverAddr }: { isConnected: boolean; serverAddr: string }) {
  ...
  const callScheduler = async (body: Record<string, any>): Promise<any> => {
    const ip    = serverConnection.getIP();
    const port  = serverConnection.getPort();
    const token = serverConnection.getToken();
    if (!ip || !port) throw new Error('No server connected');
    const ctrl = new AbortController();
    setTimeout(() => ctrl.abort(), 30_000);
    const headers: Record<string, string> = { 'Content-Type': 'application/json' };
    if (token) headers['Authorization'] = `Bearer ${token}`;
    const res = await fetch(`http://${ip}:${port}/api/scheduler`, {
      method: 'POST',
      headers,
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      const data = await callScheduler({ action: 'list' });
  ...
  const toggleTask = async (id: string) => {
    haptics.light(); setActionId(id);
    try { await callScheduler({ action: 'toggle', id }); await loadTasks(); }
  ...
  const deleteTask = (id: string) => {
    Alert.alert('Delete Task', 'Remove this scheduled task?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        ...
        try { await callScheduler({ action: 'delete', id }); await loadTasks(); }
        catch (e: any) { Alert.alert('Error', e?.message); }
        finally { setActionId(null); }
      }},
    ]);
  ...
  const runNow = async (id: string) => {
    haptics.medium(); setActionId(id);
    try {
      await callScheduler({ action: 'run_now', id });
      haptics.success();
      Alert.alert('Queued', 'Task will run within 30 seconds.');
  ...
  const statusColor = (s: string) => (
    { ok: '#00FF88', error: '#FF3366', timeout: '#FFB800', blocked: '#FF3366', pending: '#4A6A7A' }[s] || '#4A6A7A'
  );

  return (
    <View>
      <NexusTitle main="SCHEDULED " accent="TASKS" accentColor={N.green} />

      {/* Offline warning */}
      {!isConnected ? (
        <View style={sch.offlineBox}>
          <MaterialIcons name="wifi-off" size={16} color={N.red} />
          <Text style={{ color: N.red, fontFamily: MONO, fontSize: 12 }}>Server offline — tasks paused</Text>
        </View>
      ) : (
        <View style={[sch.offlineBox, { borderColor: N.green + '40', backgroundColor: N.green + '08' }]}>
          <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: N.green }} />
          <Text style={{ color: N.green, fontFamily: MONO, fontSize: 12, flex: 1 }}>Scheduler active · {serverAddr}</Text>
          <TouchableOpacity onPress={loadTasks} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <MaterialIcons name="refresh" size={16} color={N.green} />
          </TouchableOpacity>
        </View>
      )}
      ...
      {/* Error */}
      {error ? (
        <Text style={{ color: N.red, fontFamily: MONO, fontSize: 11, marginBottom: 10 }}>{error}</Text>
      ) : null}

      {/* Tasks list */}
      {loading ? (
        <View style={{ alignItems: 'center', paddingVertical: 24 }}>
          <ActivityIndicator color={N.green} />
          <Text style={{ color: N.textDim, fontFamily: MONO, fontSize: 11, marginTop: 8 }}>Loading tasks...</Text>
        </View>
      ) : tasks.length === 0 ? (
        <View style={sch.emptyState}>
          <Text style={{ fontSize: 32, marginBottom: 8 }}>⏰</Text>
          <Text style={sch.emptyTitle}>No scheduled tasks yet</Text>
          <Text style={sch.emptyHint}>Tap a preset below to automate your PC</Text>
        </View>
      ) : (
        tasks.map(task => {
          const sCol   = statusColor(task.lastStatus);
          const nextTs = task.enabled && task.nextRun > 0
            ? `Next: ${new Date(task.nextRun * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
            : 'Paused';
          const isActing = actionId === task.id;
          return (
            <View key={task.id} style={[sch.taskCard, { borderColor: task.enabled ? 'rgba(0,229,255,0.15)' : 'rgba(255,255,255,0.05)' }]}>
              {/* Header */}
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={sch.taskName} numberOfLines={1}>{task.name}</Text>
                  <Text style={sch.taskMeta}>Every {task.interval} {task.unit} · {nextTs}</Text>
                </View>
                {/* Toggle */}
                <TouchableOpacity
                  onPress={() => toggleTask(task.id)}
                  disabled={isActing}
                  style={[sch.toggle, { backgroundColor: task.enabled ? 'rgba(0,255,136,0.2)' : 'rgba(255,255,255,0.06)' }]}
                >
                  {isActing
                    ? <ActivityIndicator size="small" color={N.green} style={{ transform: [{ scale: 0.6 }] }} />
                    : <View style={[sch.thumb, { backgroundColor: task.enabled ? '#00FF88' : '#3A5060', alignSelf: task.enabled ? 'flex-end' : 'flex-start' }]} />
                  }
                </TouchableOpacity>
              </View>
              {/* Status */}
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 8 }}>
                <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: sCol }} />
                <Text style={[sch.statusTxt, { color: sCol }]}>
                  {task.lastStatus === 'pending' ? 'Waiting for first run' : `${task.lastStatus.toUpperCase()} · ${task.runCount} runs`}
                </Text>
              </View>
              {/* Last output if error */}
              {task.lastOutput && task.lastStatus !== 'ok' ? (
                <Text style={sch.lastOutput} numberOfLines={3}>{task.lastOutput}</Text>
              ) : null}
              {/* Actions */}
              <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                <TouchableOpacity
                  onPress={() => runNow(task.id)}
                  disabled={isActing}
                  style={sch.runBtn}
                >
                  <MaterialIcons name="play-arrow" size={14} color="#00E5FF" />
                  <Text style={sch.runBtnTxt}>RUN NOW</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => deleteTask(task.id)} disabled={isActing} style={sch.deleteBtn}>
      ...
      <View style={{ marginTop: 16 }}>
        <Text style={sch.presetsLabel}>QUICK ADD</Text>
        <View style={sch.presetsGrid}>
          {PRESETS.map((p, i) => (
            <TouchableOpacity
              key={i}
              onPress={() => addPreset(p)}
              ...
              activeOpacity={0.8}
              style={[sch.presetBtn, { opacity: adding || !isConnected ? 0.5 : 1 }]}
            >
              <Text style={{ fontSize: 16 }}>{p.icon}</Text>
              <View style={{ flex: 1 }}>
                <Text style={sch.presetName}>{p.name}</Text>
                <Text style={sch.presetMeta}>Every {p.interval} {p.unit}</Text>
              </View>
              {adding ? <ActivityIndicator size="small" color={N.green} style={{ transform: [{ scale: 0.7 }] }} /> : <MaterialIcons name="add" size={14} color={N.green} />}
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      {/* Info note */}
      <View style={[s2.noteBox, { marginTop: 14 }]}>
        <MaterialIcons name="info-outline" size={12} color={N.teal} />
        <Text style={[s2.noteTxt, { color: N.teal + 'AA' }]}>Tasks run on your PC server. Minimum interval is 5 minutes. Tasks persist across server restarts.</Text>
      </View>
    </View>
  );
}
...
const sch = StyleSheet.create({
  offlineBox: { flexDirection:'row', alignItems:'center', gap:8, borderWidth:1, borderColor:'rgba(255,68,68,0.4)', backgroundColor:'rgba(255,68,68,0.08)', borderRadius:8, paddingHorizontal:12, paddingVertical:10, marginBottom:14 },
  emptyState: { alignItems:'center', paddingVertical:30 },
  emptyTitle: { color:'#4A6A7A', fontFamily:MONO, fontSize:13, textAlign:'center' },
  emptyHint:  { color:'#2A3A4A', fontFamily:MONO, fontSize:11, textAlign:'center', marginTop:4 },
  taskCard:   { backgroundColor:'#0C1220', borderRadius:12, borderWidth:1, padding:14, marginBottom:10 },
  taskName:   { color:'#D8E8F0', fontFamily:MONO, fontSize:14, fontWeight:'700' },
  taskMeta:   { color:'#4A6A7A', fontFamily:MONO, fontSize:11, marginTop:2 },
  ...
  statusTxt:  { fontFamily:MONO, fontSize:10 },
  lastOutput: { color:'#4A6A7A', fontFamily:MONO, fontSize:10, marginTop:6, paddingTop:6, borderTopWidth:1, borderTopColor:'rgba(255,255,255,0.04)', lineHeight:16 },
  runBtn:     { flex:1, height:34, borderRadius:8, backgroundColor:'rgba(0,229,255,0.08)', alignItems:'center', justifyContent:'center', flexDirection:'row', gap:6 },
  runBtnTxt:  { color:'#00E5FF', fontFamily:MONO, fontSize:11, fontWeight:'700' },
  deleteBtn:  { width:34, height:34, borderRadius:8, backgroundColor:'rgba(255,51,102,0.08)', alignItems:'center', justifyContent:'center' },
  presetsLabel:{ color:'#4A6A7A', fontFamily:MONO, fontSize:11, letterSpacing:1, marginBottom:8 },
  presetsGrid: { gap:8 },
  presetBtn:  { flexDirection:'row', alignItems:'center', gap:10, paddingHorizontal:14, paddingVertical:12, borderRadius:8, backgroundColor:'rgba(0,229,255,0.04)', borderWidth:1, borderColor:'rgba(0,229,255,0.12)' },
  presetName: { color:'#7A96AA', fontFamily:MONO, fontSize:12, fontWeight:'600' },
  presetMeta: { color:'#3A5060', fontFamily:MONO, fontSize:10, marginTop:2 },
});
