...
const NAV_TARGETS = [
  '/(tabs)/nexushome',  // Primary — explicit nexushome segment
  '/(tabs)',            // Tabs root (index.tsx re-exports nexushome)
  '/main-menu',         // main-menu.tsx fires its own 7-path retry
  '/',                  // Last resort root
] as const;
...
function attemptNavigation(
  ...
  // Path 0a: layout state flip (most reliable — layout owns routing decision)
  try {
    const setNeeds = (global as any).__setNeedsOnboarding;
    if (typeof setNeeds === 'function') {
      setNeeds(false);
      console.log('[exitOnboarding] global __setNeedsOnboarding(false) fired');
    }
  } catch {}

  // Path 0b: module-level router ref in _layout
  try {
    const fn = (global as any).__onboardingComplete;
    if (typeof fn === 'function') {
      fn();
      console.log('[exitOnboarding] global __onboardingComplete fired');
    }
  } catch {}

  // Paths 1-4: try replace() then navigate() for each NAV_TARGET
  for (const target of NAV_TARGETS) {
    try {
      router.replace(target as Parameters<Router['replace']>[0]);
      console.log(`[exitOnboarding] router.replace("${target}") succeeded`);
      return target;
    } catch (navErr) {
      console.warn(`[exitOnboarding] router.replace("${target}") failed:`, navErr);
    }
    // Fallback: try navigate() for the same target
    try {
      if (typeof (router as any).navigate === 'function') {
        (router as any).navigate(target);
        console.log(`[exitOnboarding] router.navigate("${target}") succeeded`);
        return target;
      }
    } catch {}
  }

  // Path 5: deferred micro-task re-fire of globals (catches timing races)
